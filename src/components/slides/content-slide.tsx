  ...
  return (
    <div className="relative flex h-full w-full px-16 py-12 overflow-hidden noise-overlay">
      ...
      <div className="absolute inset-0 pointer-events-none">
        {/* Subtle radial glow */}
        <div className="absolute top-[20%] left-[10%] w-[600px] h-[400px] bg-gradient-radial from-accent-blue/[0.04] via-transparent to-transparent blur-3xl" />
        {/* Secondary accent */}
      ...
      <div className={`relative z-10 flex flex-col justify-center ${slide.sideImage ? "flex-1" : "w-full"}`}>
        ...
        {!slide.sideImage && (
          <div className="absolute top-0 right-0 w-48 h-48 pointer-events-none">
            ...
            <div className="absolute top-10 right-10 w-3 h-3 border-t border-r border-accent-blue-light/20" />
          </div>
        )}
        ...
        {slide.bullets && (
          <ul className="mt-8 space-y-4 max-w-4xl">
            {slide.bullets.map((bullet, i) => (
              ...
              >
                <span className="mt-[0.6em] h-1.5 w-1.5 shrink-0 rounded-full bg-accent-blue-light/50 ring-4 ring-accent-blue-light/10" />
                <span>{bullet}</span>
              </li>
            ))}
          </ul>
          ...
          <div className="mt-8 space-y-5 max-w-4xl">
            {slide.numberedSteps.map((step, i) => (
              <div key={i} className="flex items-start gap-5 group">
                <div className="shrink-0 w-8 h-8 rounded-lg bg-accent-blue/10 border border-accent-blue-light/20 flex items-center justify-center">
                  <span className="font-code text-sm font-bold text-accent-blue-light">
                    {i + 1}
                  </span>
                </div>
                <div className="pt-1">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3 max-w-5xl">
            {slide.cards.map((card, i) => (
              ...
              >
                ...
                {/* Card accent line */}
                <div className="w-8 h-0.5 bg-gradient-to-r from-accent-blue-light/50 to-transparent mb-4" />
                  ...
                  <ul className="mt-3 space-y-1.5">
                    {card.body.split(" • ").map((item, idx) => (
                      <li key={idx} className="flex items-start gap-2 text-[clamp(0.95rem,1.2vw,1.05rem)] leading-relaxed text-deck-muted">
                        <span className="mt-[0.5em] h-1 w-1 shrink-0 rounded-full bg-accent-blue-light/40" />
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
        ...
        {slide.callout && (
          <div className="mt-10 max-w-4xl">
            <div className="flex items-start gap-4 rounded-lg border border-accent-blue/20 bg-accent-blue/[0.04] px-5 py-4">
              <div className="shrink-0 mt-0.5 w-1 h-full min-h-[1.5rem] rounded-full bg-accent-blue-light/40" />
